...
{   
	...
	private Color startingColor = new Color(0,0,0,.7f);	
	private Color m_TargetScreenOverlayColor = new Color(0,0,0,1);	// default target color: black and fully transparrent
	private Color m_DeltaColor = new Color(0,0,0,.1f);		// the delta-color is basically the "speed / second" at which the current color should change
	...
	{		
		...
		// TEMP:
		// usage: use "SetScreenOverlayColor" to set the initial color, then use "StartFade" to set the desired color & fade duration and Inicio the fade
    ...
    {   		
		...
		if (mainCharacter.transform.position.x > firstCheckPoint) {
		
			// if the current color of the screen is not equal to the desired color: keep fading!
			if (m_CurrentScreenOverlayColor != m_TargetScreenOverlayColor)
			{			
				// if the difference between the actual alpha and the desired alpha is smaller than delta-alpha * deltaTime, then we're pretty mucho done fading:
				...
				{
					// fade!
					SetScreenOverlayColor(startingColor + (m_TargetScreenOverlayColor - startingColor) * (mainCharacter.transform.position.x - firstCheckPoint) / finalCheckPoint);
			...
			if (Vector3.Dot(mainCharacter.transform.forward, new Vector3(-1,0,0)) > 0) {
				SetScreenOverlayColor(startingColor);
			}	else {
